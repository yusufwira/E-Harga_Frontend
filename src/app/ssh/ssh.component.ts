...
export class SshComponent implements OnInit {
  ...
  constructor(public db:DatabaseServiceService,private dialogService: NbDialogService) { }
  datas = [];
  ...
  ngOnInit():void {
   ...
   deleterSsh(dialog: TemplateRef<any>,kode) {
    this.dialogService.open(dialog, { context: kode});
  }



  DetailSsh(dialog: TemplateRef<any> , kode, hasScroll: boolean) {
     this.db.kode = kode;
  ...
  DeleteSsh(kode){
    this.db.kode = kode;
    this.db.deleteSSH().subscribe((data)=>{
       this.ngOnInit()
      console.log(data);
    })
